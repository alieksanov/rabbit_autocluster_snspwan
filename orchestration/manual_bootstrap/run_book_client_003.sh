...
Restart=on-failure
RestartSec=10
ExecStart=/usr/bin/systemd-nspawn --setenv "RABBITMQ_USE_LONGNAME=true" -D /var/lib/machines/rabbit_autocluster --bind-ro /etc/resolv.conf -M rabbitsensu-2-003.shared.adm.las1.mz-inc.com /launch.sh /usr/lib/rabbitmq/sbin/rabbitmq-server
ExecStop=machinectl terminate rabbitsensu-2-003.shared.adm.las1.mz-inc.com
SuccessExitStatus=69
...
cd /var/lib/machines && ln -s consul_rabbit* consul_rabbit
cd /var/lib/machines &&ln -s rabbit_autocluster* rabbit_autocluster
...
cat <<EOFCC > /var/lib/machines/consul_rabbit/consul/config/config.json
{
  ...
  "enable_syslog": false,
  "enable_debug": true,
  "node_name": "rabbitsensu-2-003.shared.adm.las1.mz-inc.com",
  "server": false,
...
[
...

  {
    ...
    [
      ...
      {consul_port, 8500},
      {consul_svc, "rabbitmq"},
      {consul_svc_addr, "rabbitsensu-2-003.shared.adm.las1.mz-inc.com"},
